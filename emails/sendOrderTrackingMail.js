...
const { orderTrackingTemplate } = require("../templates/orderTrackingTemplate");

const sendOrderTrackingMail = async (orderId) => {
  try {
    ...
    await gmailTransporter.sendMail({
      ...
      subject: "Order Shipped! Track Your Order",
      html: orderTrackingTemplate
        .replaceAll("{orderId}", orderId)
        .replaceAll("{trackingId}", order.trackingId)
        .replaceAll("{trackingCompany}", order.trackingCompany)
        .replaceAll("{trackingLink}", order.trackingLink)
...
module.exports = sendOrderTrackingMail;
